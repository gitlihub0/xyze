...
def main():
    ...
    pkg = input(f"Paket: ").strip() or "com.gitlihub0"
    if not pkg.startswith("com."): pkg = f"com.{pkg}"
    user = input("Yazar: ").strip() or "gitlihub0"
    ...
    o_pkg, n_pkg = os.path.join(root, "com", "gitlihub0"), os.path.join(root, *pkg.split('.'))
    ...
    for r, ds, fs in os.walk(dst, topdown=False):
        for f in fs:
            ...
            try:
                ...
                c = c.replace("@gitlihub0", f"@{user}").replace("New", name)
